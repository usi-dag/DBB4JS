...
public class Query12SqlGroupByAndSingleFilter implements Query {
    @Override
    public List<?> execute(TPCHStreamDB db) {
        Date min = new Date("1994-01-01");
        Date max = new Date("1995-01-01");
        Comparator<Query12Imperative.Result> comparator = Comparator.comparing(row -> row.l_shipmode);
        return SQLStreamUtils
                .streamJoin(
                        db.lineitem().filter(
                                l -> (l.l_shipmode().equals("MAIL") || l.l_shipmode().equals("SHIP"))
