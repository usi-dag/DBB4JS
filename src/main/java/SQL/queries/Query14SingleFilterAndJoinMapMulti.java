...
public class Query14SingleFilterAndJoinMapMulti implements Query {
    @Override
    public List<?> execute(TPCHStreamDB db) {
        ...
        var lineItemPartJoin = SQLStreamUtils.streamJoinMapMulti(
                        db.lineitem().filter(x -> x.l_shipdate().compareTo(dateFrom) >= 0 && x.l_shipdate().compareTo(dateTo) < 0),
