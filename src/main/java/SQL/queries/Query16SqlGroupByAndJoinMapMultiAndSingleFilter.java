...
public class Query16SqlGroupByAndJoinMapMultiAndSingleFilter implements Query {
    @Override
    public List<?> execute(TPCHStreamDB db) {
        ...
        var filteredPart = db.part()
                .filter(p -> !p.p_brand().equals("Brand#45") && !p.p_type().startsWith("MEDIUM POLISHED") && sizes.contains(p.p_size()));
